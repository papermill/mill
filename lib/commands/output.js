...
function output (lepath, callback) {
  ...
  lepath = path.resolve(lepath) || process.cwd();
  
  app.log.debug("path: ", lepath);
  ...
  async.waterfall([
      ...
      function getUserConf(callback) {
        ...
        readConfig(lepath, app.config.get(), function (err, config) {
